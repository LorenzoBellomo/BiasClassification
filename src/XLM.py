...
def train_model(df, model_type, _type, _field):
    ...
    '''
    # XLMTokenizer: tokenizing texts and turning them into integers vectors
    ...
    '''
    Uploading XLM Model and defining parameters
    '''
    # XLM Pre-trained Model: bert-base-uncased is a smaller pre-trained model
    ...
    for epoch in range(1, epochs+1):
        ...
        # TODO change path
        torch.save(model.state_dict(), f'/homenfs/l.bellomo1/datasets/new_attempt_classifier/explainability/models/{model_type}/{_field}/{_type}/finetuned_XLM6class_epoch_{epoch}.model')
    ...
    print(df_stats)
    df_stats.to_csv(f'/homenfs/l.bellomo1/datasets/new_attempt_classifier/explainability/models/{model_type}/{_field}/{_type}/XLM_training_stats.csv')    # plot Learning Curve
    plt.rcParams["figure.figsize"] = (12,6)
    plt.plot(df_stats['Training Loss'], 'b-o', label="Training")
    plt.plot(df_stats['Valid. Loss'], 'g-o', label="Validation")
    plt.title("XLM RoBERTa - Learning Curve")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.xticks([1, 2, 3])
    plt.savefig(f'/homenfs/l.bellomo1/datasets/new_attempt_classifier/explainability/models/{model_type}/{_field}/{_type}/XLM_learningcurve.png') 
    plt.show()
...
### XLM maintext training punt
print('\n')
print('----------------------------------')
print('XLM maintext training punt')
print('----------------------------------')
print('\n')
...
### XLM maintext training no punt
print('\n')
print('----------------------------------')
print('XLM maintext training no punt')
print('----------------------------------')
print('\n')
...
### XLM title training no punt
print('\n')
print('----------------------------------')
print('XLM title training no punt')
print('----------------------------------')
print('\n')
df = pd.read_csv('/homenfs/l.bellomo1/datasets/new_attempt_classifier/datasets/training_cleaned_nopunt.csv')
#df = df.groupby('political_leaning').apply(lambda x: x.sample(n=10))
...
print('\n')
print('----------------------------------')
print('XLM all_text (title + maintext) training punt')
print('----------------------------------')
print('\n')
df = pd.read_csv('/homenfs/l.bellomo1/datasets/new_attempt_classifier/datasets/training_punt_cleaned.csv')
#df = df.groupby('political_leaning').apply(lambda x: x.sample(n=10))
...
print('\n')
print('----------------------------------')
print('XLM all_text (title + maintext)  training no punt')
print('----------------------------------')
print('\n')
df = pd.read_csv('/homenfs/l.bellomo1/datasets/new_attempt_classifier/datasets/training_cleaned_nopunt.csv')
#df = df.groupby('political_leaning').apply(lambda x: x.sample(n=10))
